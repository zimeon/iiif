...
def _parse_tiles(info, json_data):
    ...
    if (len(json_data) == 1):
        ...
        raise IIIFInfoError(
            "FIXME - support for multiple tile sizes not imeplemented")
    return json_data
...
CONF = {
    ...
    '2.0': {
        ...
        'array_params': set(['sizes', 'tiles', 'service', 'scale_factors', 'formats', 'qualities', 'supports']),
        'complex_params': {
            'sizes': _parse_noop,
            'tiles': _parse_tiles,
...
class IIIFInfo(object):
    ...
    def level(self, value):
        ...
        """
        self.profile = self.profile_prefix + \
            ("%d" % value) + self.profile_suffix
