"""Test code for iiif.auth_clickthrough.
...
class TestAll(unittest.TestCase):
    ...
    def test01_init(self):
        ...
        self.assertEqual(auth.auth_pattern, 'clickthrough')
        self.assertTrue(re.match(r'\d+_', auth.cookie_prefix))
        auth = IIIFAuthClickthrough(cookie_prefix='abc')
        self.assertEqual(auth.cookie_prefix, 'abc')

    def test02_login_service_description(self):
        """Test login_service_description."""
        auth = IIIFAuthClickthrough()
        desc = auth.login_service_description()
        self.assertEqual(auth.confirm_label, desc['confirmLabel'])

    def test03_login_handler(self):
        """Test login_handler."""
        with dummy_app.test_request_context('/a_request'):
            ...
            self.assertTrue(
                re.search(auth.account_cookie_name + '=clickthrough-ok;',
                          set_cookie))
